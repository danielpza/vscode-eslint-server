...
namespace ProbeFailedRequest {
  export const type = new RequestType<ProbeFailedParams, void, void, void>(
    "eslint/probeFailed"
  );
}
...
namespace RuleData {
  export function hasMetaType(
    value: RuleData | undefined
  ): value is RuleData & { meta: { type: string } } {
    return (
      value !== undefined &&
      value.meta !== undefined &&
      value.meta.type !== undefined
    );
  }
}
...
function makeDiagnostic(problem: ESLintProblem): Diagnostic {
  ...
  const startLine = Is.nullOrUndefined(problem.line)
    ? 0
    : Math.max(0, problem.line - 1);
  const startChar = Is.nullOrUndefined(problem.column)
    ? 0
    : Math.max(0, problem.column - 1);
  const endLine = Is.nullOrUndefined(problem.endLine)
    ? startLine
    : Math.max(0, problem.endLine - 1);
  const endChar = Is.nullOrUndefined(problem.endColumn)
    ? startChar
...
const languageId2ParserRegExp: Map<
  string,
  RegExp[]
...
const document2Settings: Map<string, Promise<TextDocumentSettings>> = new Map<
  string,
...
function resolveSettings(
  ...
  resultPromise = connection.workspace
    ...
    .then((configuration: ConfigurationSettings) => {
      ...
      settings.resolvedGlobalPackageManagerPath = globalPathGet(
      ...
      const workingDirectoryConfig = configuration.workingDirectory ?? {
        ...
        async (libraryPath) => {
          ...
          if (library === undefined) {
            ...
            if (library === undefined) {
              ...
              settings.validate = Validate.off;
              connection.console.error(
                `The eslint library loaded from ${libraryPath} doesn\'t export a CLIEngine. You need at least eslint@1.0.0`
              );
            } else {
          ...
          if (
            ...
            if (filePath !== undefined) {
              const parserRegExps = languageId2ParserRegExp.get(
                document.languageId
              );
              const pluginName = languageId2PluginName.get(document.languageId);
              const parserOptions = languageId2ParserOptions.get(
                document.languageId
              );
              if (defaultLanguageIds.has(document.languageId)) {
                ...
                const eslintConfig: ESLintConfig | undefined = withCLIEngine(
                  (cli) => {
                ...
                if (eslintConfig !== undefined) {
                  ...
                  if (parser !== undefined) {
                    ...
                    if (
                      settings.validate !== Validate.on &&
                      parserOptions !== undefined &&
                      typeof eslintConfig.parserOptions?.parser === "string"
                    ) {
                      for (const regExp_1 of parserOptions.regExps) {
                        if (
                          regExp_1.test(parser) &&
                          (parserOptions.parsers.has(
                            eslintConfig.parserOptions.parser
                          ) ||
                            (parserOptions.parserRegExps !== undefined &&
                              parserOptions.parserRegExps.some((parserRegExp) =>
                                parserRegExp.test(
                                  eslintConfig.parserOptions!.parser!
                                )
                              )))
                  ...
                  if (
                    settings.validate !== Validate.on &&
                    Array.isArray(eslintConfig.plugins) &&
                    eslintConfig.plugins.length > 0 &&
                    pluginName !== undefined
                  ) {
  ...
  documents.onDidOpen((event) => {
    resolveSettings(event.document).then((settings) => {
      ...
      if (settings.format) {
        ...
        let pattern: string = isFile ? uri.path.replace(/\\/g, "/") : uri.path;
...
async function validateSingle(
  ...
  const settings = await resolveSettings(document);
  if (
    settings.validate !== Validate.on ||
    !TextDocumentSettings.hasLibrary(settings)
  ) {
...
function validate(
  ...
  withCLIEngine((cli) => {
    codeActions.delete(uri);
    const report: ESLintReport = cli.executeOnText(
      content,
      file,
      settings.onIgnoredFiles !== ESLintSeverity.off
    );
...
function withCLIEngine<T>(
  func: (cli: CLIEngine) => T,
  settings: TextDocumentSettings & { library: ESLintModule },
  options?: CLIOptions
): T {
  ...
  try {
    if (settings.workingDirectory) {
      ...
      if (
        ...
        fs.existsSync(settings.workingDirectory.directory)
      ) {
  ...
  (params) => {
    ...
    function createDisableLineTextEdit(
      ...
      return TextEdit.insert(
        ...
        `${indentationText}// eslint-disable-next-line ${editInfo.ruleId}${EOL}`
      );
    }
    ...
    function createDisableFileTextEdit(editInfo: Problem): TextEdit {
      ...
      return TextEdit.insert(
        ...
        `/* eslint-disable ${editInfo.ruleId} */${EOL}`
      );
    }
    ...
    return resolveSettings(textDocument).then(
      async (settings): Promise<CodeAction[]> => {
        ...
        if (isSourceFixAll || isSource) {
          if (isSourceFixAll && settings.codeActionOnSave.enable) {
            const textDocumentIdentifer: VersionedTextDocumentIdentifier = {
              uri: textDocument.uri,
              version: textDocument.version,
            };
            ...
            if (edits !== undefined) {
              result.fixAll.push(
                CodeAction.create(
                  ...
                  {
                    documentChanges: [
                      TextDocumentEdit.create(textDocumentIdentifer, edits),
                    ],
                  },
                  ESLintSourceFixAll
                )
          ...
          if (Problem.hasSuggestions(editInfo)) {
            editInfo.suggestions.forEach((suggestion, suggestionSequence) => {
              ...
              workspaceChange
                .getTextEditChange({ uri, version: documentVersion })
                .add(
                  SuggestionsProblem.createTextEdit(textDocument, suggestion)
                );
              changes.set(
        ...
        if (result.length > 0) {
          ...
          sameProblems.forEach((same, ruleId) => {
            if (same.length > 1) {
              ...
              same
                .map((fix) => FixableProblem.createTextEdit(textDocument, fix))
      ...
      (cli) => {
        ...
        if (
          ...
          if (
            ...
            report.results[0].output !== undefined
          ) {
